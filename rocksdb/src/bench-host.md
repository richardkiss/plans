...
on. This page describes each host, so numbers can be compared across
machines. New hosts (a slower box is planned) get a new section here.

## host-1 (2026-07-15 runs)

The host for all published results so far. It is a virtual machine backed by
...
- The CPU is fast; the disk is the deliberate bottleneck. The engine gap in
  the benchmark is mostly an I/O-pattern story, so disk type matters far
  more than CPU here.
...
- Virtualization adds I/O overhead but applies equally to all four backends.
